...
while IFS= read -r CONFIG || [[ -n "${CONFIG}" ]]; do
    ...
    if ! uv run python -u detection.py \
        ...
        echo "Skip ${CONFIG}: detection failed. Check GPU memory, model files, gated model access, HF download/cache state, span mapping, or model-specific runtime support." >> "${OUTPUT_FILE}"
